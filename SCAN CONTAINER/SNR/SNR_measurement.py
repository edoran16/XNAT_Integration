...
def main(argv):
    ...
    sd, pn = mf.dicom_geo(fulldicomfile)
    ...
    mf.snr_analysis(fulldicomfile, imdata, img, outputpath, caseT, caseS, caseC)
...
#
#     pixel_dims, slice_thickness, no_averages, pixelBW, freqX, TRep, No_PEsteps = mf.snr_meta(dcmfile)
#     print(pixel_dims, slice_thickness, no_averages, pixelBW, freqX, TRep, No_PEsteps)
#
#     # mask phantom and background
#     mask, bin_mask = mf.create_2D_mask(img, imagepath=outpath)  # boolean and binary masks
#
#     # draw signal ROIs
#     pc_row, pc_col, quad_centres, marker_im = mf.draw_signal_ROIs(bin_mask, img, show_bbox=False, show_quad=False, imagepath=outpath)
#
#     # get signal value
#     mean_signal, all_signals = mf.get_signal_value(imdata, pc_row, pc_col, quad_centres)
#     factor = 0.655  # for single element coil, background noise follows Rayleigh distribution IPEM Report 112
#
#     # draw background ROIs
#     bROIs = mf.draw_background_ROIs(mask, marker_im, pc_col, cT, cS, cC, imagepath=outpath)
#
#     # get background/noise value
#     b_noise, all_noise = mf.get_background_noise_value(imdata, bROIs)
#
#     # SNR calculation (background method)
#     SNR_background = mf.calc_SNR(factor, mean_signal, b_noise)
#
#     # Normalised SNR calculation
#     Qfact = 1
#     NSNR, BWcorr, PixelCorr, TimeCorr, TotalCorr = mf.calc_NSNR(pixel_dims, slice_thickness, No_PEsteps, TRep, no_averages,
